...
/** Ensures the userland tools required by Apptainer's unprivileged installer (`curl`, `rpm2cpio`, `cpio`) are available
  * *inside the backend* — without needing root.
  *
  * A minimal WSL2/Lima distro frequently lacks `rpm2cpio` (an RPM-world tool that Debian/Ubuntu never ship) and
  * sometimes `curl`/`cpio`, and installing them via `apt`/`dnf` would require root, defeating the user-mode goal. So
  * Scalapptainer vendors static Linux binaries of these tools as JAR resources and, for any tool not already present on
  * the backend PATH, materialises the vendored copy into the backend cache directory.
  *
  * Already-present system copies are always preferred; the vendored binaries are a fallback only. The result is a PATH
  * prefix (the directory containing whatever was materialised) that the installer is run with, or `None` if every tool
  * was already present.
  */
