...
/** A fully-specified host process invocation.
  *
  * @param argv  the host argv (already wrapped for the backend, if any)
  * @param stdin optional text to feed to the process' standard input
  * @param cwd   optional working directory
  * @param env   extra environment variables (added to the inherited environment)
  */
...
/** Abstraction over actually executing a host command.
  *
  * The real implementation ([[OsLibRunner]]) shells out via os-lib. Tests inject
  * a recording/stub runner so that all argv-building and backend logic can be
  * exercised without a real Apptainer, WSL2 or Lima present.
  */
trait CommandRunner {
  ...
  /** Run with the parent process' stdio inherited (for interactive sessions such
    * as `apptainer shell`). Returns the exit code. The default implementation
    * falls back to capturing [[run]]; real runners override it.
    */
