...
object ApptainerInstaller {
  ...
  /** The Apptainer release this Scalapptainer version pins. The value is defined once in `build.mill`
    * (`Versions.apptainer`), emitted into the `scalapptainer/build.properties` classpath resource by Mill, and read
    * back here; bump it there in lockstep with a Scalapptainer release to adopt a new Apptainer (e.g. to pick up an
    * upstream fix). The new version installs into its own `~/.scalapptainer/<version>` directory.
    */
    ...
    Option(getClass.getResourceAsStream(resource)) match {
      ...
      case None =>
        throw new IllegalStateException(
          s"Missing build resource '$resource' (generated by Mill from Versions.apptainer)"
        )
    }
  ...
  /** Compare two dotted version strings numerically: negative if `a < b`, zero if equal, positive if `a > b`. Numeric
    * components are compared as integers (missing trailing components count as 0, so `1.5` == `1.5.0`). When the
    * numeric parts are equal, a pre-release/build suffix (e.g. `-rc.1`) sorts *below* the plain release, matching
    * SemVer.
    */
